...
module.exports = zeroyt7 = async (zeroyt7, mek, _welkom) => {
	try {
		...
		let ownerNumber = [`${owner}@s.whatsapp.net`, `6288211974450@s.whatsapp.net`]
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		mess = {
			...
			success: 'Done jangan lupa desah',
			error: {
...
varr =
`Hallo ${pushname} Nekopoi Bot have a nice day`
lett =
`𝐁𝐨𝐭 𝐈𝐧𝐟𝐨
Creator : Samuel Gans
Prefix : Multi Prefix
...
`wibu`,`upsw`]
               listmenuu = [`〽️ Sc Bot`,`☕ Speed`,`💻 Group Menu`,`📂 Sticker Menu`,`💌 Random Menu`,`💣 Owner Menu`,`🎈 Maker Menu`,`🗞️ Islami Menu`,
`☕ Download Menu`,`☕ Gabut Menu`,`☕ Search Menu`,`☕ Stalk Menu`,`☕ ️asupan Menu`,`📋 Nsfw Menu`,
`☕ Wibu Menu`,`☕ Upsw Menu`]
               nombor = 1
               startnum = 0
...
costum(buffer, sticker, Verived, `Kucing peduli`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `Kucing peduli`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
zeroyt7.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Kucing peduli' })
fs.unlinkSync(ran)
})
break
...
zeroyt7.sendMessage(`6288211974450@s.whatsapp.net`, options, text, { quoted: ftrol })
...
case 'sc':
teks = 
`*Bot Ini Menggunakan Sourcecode Private*
╭─────────────────────
├ Sc Ori = Nekopoi.care
├ Sc Full Fitur = Kucing peduli
├─────────────────────
├ Creator Base = Samuel
├ Creator Sc = Samuel Anjay
└─────────────────────`
zeroyt7.sendMessage(from, teks, text, {quoted: ftrol})
break
...
case 'harrypotter':
if (args.length == 0) return reply(`Teks Nya Mana ?\nContoh: ${prefix + command} Samuel`)
zerr = args.join(" ")
buffer = await getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=${zero}&text=${zerr}`)
zeroyt7.sendMessage(from, buffer, image, {quoted: ftrol, caption: '*©Creator : Samuel*'})
break
case 'phkomen':
...
zeroyt7.sendMessage(from, gambar, image, { quoted: ftrol, caption: '*©Creator : Samuel*' })
})
break
...
await zeroyt7.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: ftrol, caption: 'Nih Kucing peduli'})
break
...
reply(mess.wait)
zeroyt7.sendMessage(from, gambar, image, {quoted: ftrol, caption: 'Nih Kucing peduli'})
})
break
case 'darkjoke':
...
zeroyt7.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih Kucing peduli'})
break
case 'asupancecan':
ini = await fetchJson(`https://zeroyt7-api.herokuapp.com/api/asupan/cecan?apikey=${zerkey}`)
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
zeroyt7.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih Kucing peduli'})
break
case 'asupanhijaber':
